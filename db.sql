...
CREATE TABLE IF NOT EXISTS Users (
    ...
    is_active BOOLEAN DEFAULT TRUE NOT NUL
...
-- Create Products Table (Your existing table, with a crucial new 'price' column)
CREATE TABLE IF NOT EXISTS Products (
    ...
    price DECIMAL(10, 2) NOT NULL DEFAULT 0.00, -- NEW: Price of the product
    FOREIGN KEY (producer_username) REFERENCES Users(username)
);

-- Add price column if it doesn't exist (execute this separately if Products table already exists)
-- ALTER TABLE Products ADD COLUMN price DECIMAL(10, 2) NOT NULL DEFAULT 0.00;


-- Create SupplyChainEvents Table (Your existing table)
CREATE TABLE IF NOT EXISTS SupplyChainEvents (
    ...
    FOREIGN KEY (food_item_id) REFERENCES Products(food_item_id)
);

-- NEW TABLES FOR E-COMMERCE FUNCTIONALITY --

-- Table for Orders
-- This records the overall order details
...
-- This records each product bought in a specific order
...
-- Table for User Shipping Addresses (Optional, but good for saving addresses)
...
-- IMPORTANT: You might also need to drop existing 'Purchases' table if it was created in previous attempts:
